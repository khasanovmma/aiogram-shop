...
"""
    ...
    async def update_user(self, full_name, phone_number, chat_id):
        sql = """
                UPDATE Users SET full_name=$1, phone_number=$2 
                WHERE id=$3
"""
...
"""
    ...
    async def insert_product(self, product_list):
        sql = """
                INSERT INTO Products (title, description, photo, subcategory_id, price)
                VALUES ($1, $2, $3, $4, $5);        
"""
    ...
    async def get_category(self):
        sql = """
                SELECT category_name, category_code FROM Categories;
"""
        return await self.execute(sql, fetch=True)

    async def get_subcategory(self, category_code):
        sql = """
                SELECT subcategory_name, subcategory_code 
                FROM SubCategories
                WHERE category_id = ( SELECT id FROM Categories
                WHERE Categories.category_code = $1)
"""
        return await self.execute(sql, category_code, fetch=True)

    async def get_category_name(self, category_code):
        sql = """
                SELECT category_name FROM Categories WHERE category_code = $1    
"""
        return await self.execute(sql, category_code, fetchval=True)

    async def get_product_list(self, subcategory_code):
        sql = """
                SELECT title, id FROM Products 
...
"""
    ...
    async def get_subcategory_name(self, subcategory_code):
        sql = """
                        SELECT subcategory_name FROM SubCategories WHERE subcategory_code = $1    
        """
        return await self.execute(sql, subcategory_code, fetchval=True)

    async def back_subcategory(self, subcategory_code):
        sql = """
                SELECT subcategory_name, subcategory_code 
                FROM SubCategories 
                where category_id = 
                (SELECT category_id  FROM SubCategories WHERE  subcategory_code = $1)        
"""
        return await self.execute(sql, subcategory_code, fetch=True)

    async def back_subcategory_name(self, subcategory_code):
        sql = """
...
"""
    ...
    async def get_product_by_id(self, product_id):
        ...
        return await self.execute(sql, int(product_id), fetchrow=True)

    async def get_subcategory_code_from_id(self, product_id):
        sql = """
                    SELECT subcategory_code FROM Subcategories
                    WHERE id = (SELECT subcategory_id FROM Products WHERE id = $1)
"""
        return await self.execute(sql, product_id, fetchval=True)

    async def back_subcategory_name_from_id(self, product_code):
        sql = """
                SELECT subcategory_name FROM subcategories
                WHERE id = (SELECT subcategory_id FROM Products WHERE id=$1)
"""
        return await self.execute(sql, product_code, fetchval=True)

    async def back_product_list(self, product_id):
        sql = """
                SELECT title, id FROM Products WHERE subcategory_id = 
                (SELECT subcategory_id FROM Products WHERE id = $1)
"""
        return await self.execute(sql, product_id, fetch=True)
